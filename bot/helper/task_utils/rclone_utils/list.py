...
class RcloneList:
    ...
    async def _event_handler(self):
        ...
        handler = self.listener.client.add_handler(
            CallbackQueryHandler(
                pfunc,
                filters=regex("^rcq") & user(self.listener.userId)
            ),
            group=-1,
        )
        try:
    ...
    async def get_path_buttons(self):
        ...
        if self.list_status == "rcu":
            ...
            msg += (
                f"\nDefault Rclone Path: {default_path}"
                if default_path
                else ""
            )
        msg += f"\n\nItems: {items_no}"
    ...
    async def get_path(self, itype=""):
        if itype:
            self.item_type == itype # type: ignore
        elif self.list_status == "rcu":
            self.item_type == "--dirs-only" # type: ignore
        cmd = [
            pkg_info["pkgs"][3],
            "lsjson",
            self.item_type,
    ...
    async def get_rclone_path(self, status, config_path=None):
        ...
        if (
            self.config_path != "rclone.conf"
            and not self.listener.isCancelled
        ):
