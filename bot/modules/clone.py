...
async def rcloneNode(client, message, link, dst_path, rcf, listener):
    ...
    cmd1 = ['zcl', 'lsf', '--fast-list', '-R', '--files-only', '--config', config_path, destination]
    cmd2 = ['zcl', 'lsf', '--fast-list', '-R', '--dirs-only', '--config', config_path, destination]
    cmd3 = ['zcl', 'size', '--fast-list', '--json', '--config', config_path, destination]
